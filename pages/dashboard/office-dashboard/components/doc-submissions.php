...
try {
    ...
    $getDocuments = $c->prepare("
        ...
        WHERE doc.user_id = ? AND DATE(doc.tstamp) BETWEEN ? AND ? $docStatus
        ORDER BY doc.tstamp DESC;
    ");
    $getDocuments->execute([$userId, $dateFrom, $dateTo]);
    $documents = $getDocuments->fetchAll();
?>

    <table id="tbl-doc-submissions" class="table table-hover table-sm">
        <thead>
            <tr>
                <th>Document No.</th>
                <th>Type</th>

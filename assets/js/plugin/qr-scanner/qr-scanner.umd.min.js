	...
	class e {
		...
		async start() {
			...
			if (!this._active || this._paused)
				if (("http:" !== window.location.protocol && console.warn("The camera stream is only accessible if the page is transferred via https."), (this._active = !0), !document.hidden))
		...
		static async createQrEngine(a) {
			...
			a = () => import("./assets/js/plugin/qr-scanner/qr-scanner-worker.min.js").then((c) => c.createWorker());
				...
				("requestVideoFrameCallback" in this.$video ? this.$video.requestVideoFrameCallback.bind(this.$video) : requestAnimationFrame)(async () => {
					if (!(1 >= this.$video.readyState)) {
						...
						c
							...
							: this.$codeOutlineHighlight && !this._codeOutlineHighlightRemovalTimeout && (this._codeOutlineHighlightRemovalTimeout = setTimeout(() => (this.$codeOutlineHighlight.style.display = "none"), 100));
					}
					this._scanFrame();
				});
		}
